...
<script>
$(document).ready(function(){	// html 문서가 모두 읽히고 나면(준비되면) 다음 내용을 실행 해라
	listCall();
});

	  function del(){	// 체크 표시된 내용을 delArr에 담아보자
		var delArr=[];
	...
	function drawlist(list){
		...
		for(item of list){
			...
			content += '<td><input type ="checkbox" name="del" value="'+item.idx+'" /></td>';
				...
				// java.sql.Date 는 javascript에서는 ms로 변환하여 표시한다.
				// 방법1. Back-end(개발과정에 사용, 서버를 꺼야하기에 운영중에 사용x) : DTO에 반환 날짜 타입을 문자열로 변경
				//content +='<td>'+item.reg_date+'</td>';
				// 방법2. Front-end(운영중에 사용) : js에서 직접 변환
